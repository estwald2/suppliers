...
// --- ELEMENTI DEL DOM (con aggiunte per la tabella del listino) ---
const loginSection = document.getElementById('login-section');
const searchSection = document.getElementById('search-section');
const loginForm = document.getElementById('login-form');
// ... (altri elementi invariati) ...
...
function updateUI(user) { if (user) { loginSection.classList.add('hidden'); searchSection.classList.remove('hidden'); } else { loginSection.classList.remove('hidden'); searchSection.classList.add('hidden'); } }
...
function displayResults(products) {
    resultsContainer.innerHTML = '';
    if (!products || products.length === 0) { resultsContainer.innerHTML = '<p>Nessun prodotto trovato.</p>'; return; }
    
    products.forEach(product => {
        let listinoHtml = `<table class="listino-table"><thead><tr><th>ID Item</th><th>Dimensione</th><th>Prezzo Netto</th></tr></thead><tbody>`;
        product.listino.forEach(item => { listinoHtml += `<tr><td>${item.id_item}</td><td>${item.dimensione}</td><td>€ ${item.prezzo_netto.toFixed(2)}</td></tr>`; });
        listinoHtml += '</tbody></table>';

        const productCard = document.createElement('div');
        productCard.className = 'product-card';
        productCard.innerHTML = `<div class="product-actions"><button class="action-button edit" data-id="${product.id}"><i class="fas fa-pencil-alt"></i></button><button class="action-button delete" data-id="${product.id}"><i class="fas fa-trash"></i></button></div><h2>${product.nome_generico}</h2><p><strong>Produttore:</strong> ${product.produttore}</p><p><strong>Fornitore:</strong> ${product.fornitore_nome} (<a href="mailto:${product.fornitore_email}">${product.fornitore_email}</a>)</p><h3>Listino:</h3>${listinoHtml}`;
...
function addListinoRow(item = { id_item: '', dimensione: '', prezzo_netto: '' }) {
    ...
    // Aggiungi l'evento per eliminare la riga
...
function openModal(product = null) {
    productForm.reset();
    listinoTableBody.innerHTML = ''; // Pulisci la tabella
    if (product) {
        ...
        // MODIFICA: le keywords ora sono separate da virgola, quindi le uniamo
        document.getElementById('keywords').value = product.keywords.split(' ').join(', ');
        // Popola la tabella del listino dai dati JSON
        if (product.listino && product.listino.length > 0) {
            product.listino.forEach(item => addListinoRow(item));
        } else {
            addListinoRow(); // Aggiungi una riga vuota se non c'è listino
        }
    } else {
        // Modal per AGGIUNGERE
        modalTitle.textContent = 'Aggiungi Nuovo Prodotto';
        productIdInput.value = '';
        addListinoRow(); // Aggiungi una riga vuota iniziale
    }
...
async function handleFormSubmit(e) {
    ...
    // MODIFICA: Conversione della tabella del listino in JSON
    ...
    // MODIFICA: Conversione delle keyword separate da virgola in una stringa separata da spazi
    const keywordsInput = document.getElementById('keywords').value;
    const keywordsForDB = keywordsInput.split(',').map(k => k.trim()).join(' ');
...
async function fetchProductAndOpenModal(id) {
    ...
    if (error) { alert(`Errore nel recupero del prodotto: ${error.message}`); } else { openModal(data); }
}
...
async function handleDeleteClick(e) {
    ...
    if (confirm('Sei sicuro di voler eliminare questo prodotto? L\'azione è irreversibile.')) {
        ...
        if (error) { alert(`Errore durante l'eliminazione: ${error.message}`); } else { performSearch(); }
    }
}
...
// --- EVENT LISTENERS (con aggiunta per la tabella del listino) ---
searchButton.addEventListener('click', performSearch);
searchInput.addEventListener('input', () => { clearTimeout(debounceTimer); debounceTimer = setTimeout(performSearch, 400); });
addProductButton.addEventListener('click', () => openModal());
cancelButton.addEventListener('click', closeModal);
productForm.addEventListener('submit', handleFormSubmit);
// Nuovo listener per aggiungere righe alla tabella
