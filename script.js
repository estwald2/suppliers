...
function displayResults(products) {
    recommendedDiv.innerHTML = '';
    alternativeDiv.innerHTML = '';
    if (products.length === 0) { recommendedContainer.classList.add('hidden'); alternativeContainer.classList.add('hidden'); noResultsMessage.classList.remove('hidden'); return; }
    noResultsMessage.classList.add('hidden');
    ...
    const processProduct = (product, container) => {
        ...
        if (product.listino) { product.listino.forEach(item => { const prezzo = typeof item.prezzo_netto === 'number' ? item.prezzo_netto.toFixed(2) : 'N/A'; listinoHtml += `<tr><td>${item.id_item || ''}</td><td>${item.dimensione || ''}</td><td>€ ${prezzo}</td></tr>`; }); }
...
function openModal(product = null) {
    ...
    if (product) {
        ...
        if (product.scheda_tecnica_url) { document.getElementById('scheda-tecnica-preview').textContent = `File attuale: ${product.scheda_tecnica_url.split('/').pop()}`; }
        if (product.listino && product.listino.length > 0) { product.listino.forEach(item => addListinoRow(item)); } else { addListinoRow(); }
    } else {
...
async function handleFormSubmit(e) {
    ...
    try {
        ...
        let oldProductData = {};
        if (id) { const { data, error } = await supabaseClient.from('prodotti').select('immagine_url, scheda_tecnica_url').eq('id', id).single(); if (error) throw new Error("Impossibile recuperare i dati del prodotto esistente."); oldProductData = data || {}; }
        ...
        const productData = { nome_generico: document.getElementById('nome_generico').value, produttore: document.getElementById('produttore').value, fornitore_nome: document.getElementById('fornitore_nome').value, fornitore_email: document.getElementById('fornitore_email').value, keywords: keywordsForDB, listino: listinoJSON, immagine_url: immagine_url, scheda_tecnica_url: scheda_tecnica_url, consigliato: isConsigliato, tipo_consigliato: tipoConsigliatoValue, };
        let dbError;
        if (id) { const { error } = await supabaseClient.from('prodotti').update(productData).eq('id', id); dbError = error; } else { const { error } = await supabaseClient.from('prodotti').insert([productData]); dbError = error; }
...
async function fetchProductAndOpenModal(id) { const { data, error } = await supabaseClient.from('prodotti').select('*').eq('id', id).single(); if (error) { alert(`Errore nel recupero del prodotto: ${error.message}`); } else { openModal(data); } }
...
productTypeRadios.forEach(radio => { radio.addEventListener('change', (e) => { livelloConsigliatoWrapper.style.display = e.target.value === 'consigliato' ? 'block' : 'none'; }); });
